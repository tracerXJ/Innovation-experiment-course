...
#定义的信号
TIME_TO_TAKE_PHOTO = "1"    #通知摄像树莓派可以开始拍摄
TIME_TO_STOP_VEHICLE = '3'  #通知摄像树莓派，底层小车运行结束，可以开始进行图像处理
count = 0                   #全局变量，用于增加LocalWebController的启动时延
def drive(cfg, CURRENT_PORT,model_path=None, use_joystick=False, model_type=None, camera_type='single',meta=[],TEMP=100):
    '''
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        ctr = LocalWebController(port=CURRENT_PORT)
        V.add(ctr,
    ...
    if type(ctr) is LocalWebController:
            print("You can now go to http://localhost:8887 to drive your car.")
    ...
    #raspi_raspi(V,cfg.DRIVE_LOOP_HZ)
    V.start(rate_hz=cfg.DRIVE_LOOP_HZ,
            max_loop_count=TEMP)#cfg.MAX_LOOPS)
    del V #防止重新定义一个名称相同的对象
    global count 
    count = 0 #在一次运行后，令count归零，为下一次启动做准备
    

HOST = '192.168.137.66'
PORT = 6668
ADDR = (HOST,PORT)
#sending message to the photo_raspi, informing that the car stopped right now,and wait for the next start signal
def raspi_raspi(cfg):
    ...
    CURRENT_PORT = 8886
    import random
    while(CURRENT_LOOPS < MAX_LOOPS_AAA):
        CURRENT_PORT=CURRENT_PORT+1
        TEMP = 4#random.randint(60,70)  本轮运行次数
        CURRENT_LOOPS = CURRENT_LOOPS + TEMP#累计运行次数
        drive(cfg,model_path='/home/pi/mycar/models/mypilot.h5',TEMP=TEMP,CURRENT_PORT=CURRENT_PORT)
        c.send(TIME_TO_TAKE_PHOTO.encode('utf-8'))
        while True:
            data = c.recv(1024)
            if not data:
            ...
            if data.decode('utf-8') == '2':
                print("Message Received,ready to start")
                break
    c.send(TIME_TO_STOP_VEHICLE.encode('utf-8'))
    s.close()
